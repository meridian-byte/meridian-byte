...
import { roundAndTruncate } from '@repo/utilities/number';
...
export const useServingTotals = (params?: {
  ...
  return {
    totalCarbs: params?.options?.round
      ...
      : roundAndTruncate(totals.carbs, 1),
    totalProtein: params?.options?.round
      ? Math.round(totals.protein)
      : roundAndTruncate(totals.protein, 1),
    totalFat: params?.options?.round
      ? Math.round(totals.fat)
      : roundAndTruncate(totals.fat, 1),
...
export const getFoodServingTotals = (params: {
  ...
  return {
    totalCarbs: roundAndTruncate(totalCarbs, 1),
    totalProtein: roundAndTruncate(totalProtein, 1),
    totalFat: roundAndTruncate(totalFat, 1),
    totalKcal: roundAndTruncate(totalKcal, 1),
  };
};
